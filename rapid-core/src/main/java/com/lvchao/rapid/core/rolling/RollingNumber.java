...
public class RollingNumber {
    ...
    /**
     * 创建新桶必须要加锁, 因为是一复合性的操作: 1.向数组中添加新的桶  2.把之前的桶里的统计数据清空(历史数据)
     */
    ...
    public Bucket getCurrentBucket() {
         ...
         * currentTime < currentBucket.windowStart + this.bucketSizeInMillseconds 表示当前时间在当前窗口内
         */
        if (currentBucket != null && currentTime < currentBucket.windowStart + this.bucketSizeInMillseconds) {
            return currentBucket;
        }

        //	如果没有找到符合当前窗口时间内的桶, 需要创建一个新的桶并返回
        if (newBucketLock.tryLock()) {
            try {
                ...
                //	这种情况就是数组元素都放满了, 需要利用环形数组的特性, 就是交替替换桶, 重复利用
                else {
    ...
    public static class BucketCircularArray implements Iterable<Bucket> {
        ...
        //	数组长度 = numBuckets + 1  【dataLength=7】
